...
int main(void)
{
	...
	printf("변수 b의 값 %d\n", b);
	printf("변수 c의 값 %d\n", c);
	printf("변수 f의 값 %f\n", f);  // 빨간색밑줄은 무조건 오류, 녹색 밑줄은 경고(!) 실행은 가능
	...
	// 최소값이 0
	printf("변수 unsigned char, %zu byte, 최소 %d, 최대 %u \n", sizeof(unsigned char), 0, UCHAR_MAX);
	printf("변수 unsigned short, %zu byte, 최소 %d, 최대 %u \n", sizeof(unsigned short), 0, USHRT_MAX);
	printf("변수 unsigned int, %zu byte, 최소 %d, 최대 %u \n", sizeof(unsigned int), 0, UINT_MAX);
	printf("변수 unsigned long, %zu byte, 최소 %d, 최대 %lu \n", sizeof(unsigned long), 0, ULONG_MAX);
